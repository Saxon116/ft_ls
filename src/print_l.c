...
/*   Updated: 2019/07/02 13:26:08 by jmondino         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    display_entries_l(t_entry *list_start, t_args *pargs, char *dirname)
{
    ...
    ft_total(list_start, pargs, dirname);
...
void display_extended_attr(t_entry *list_curr, t_args *pargs)
{
    if(list_curr->has_xattr > 0 && ft_iscinstr(pargs->flags, '@'))
    {
        ...
        while(list_curr->xattr[i])
        {
            ...
            print_spaces(28 - ft_strlen(list_curr->xattr[i]) - num_length(list_curr->xattr_sizes[i]));
